...
import {
    ...
    leadStatusOptions,LEADS_WORKLOG_SUBCOLLECTION,leadLogTypeOptions,SIMPLE_CONSIGNMENT_COLLECTION_PATH
} from './config.js';
...
const leadsGridOptions = {
    ...
    columnDefs: [
        ...
        },
        {
            ...
            cellEditor: 'agSelectCellEditor',
            cellEditorParams: {
                values: leadStatusOptions // Uses the array from your config.js
            },
            cellRenderer: params => {
                const colors = {
                ...
                return `<span class="px-2 py-1 rounded-full text-xs font-bold ${colorClass}">${params.value}</span>`;
            }
        },
        {
            field: "enquiryDate",
            ...
            valueGetter: p => p.data.enquiryDate?.toDate ? p.data.enquiryDate.toDate() : p.data.enquiryDate
        },
        { 
        ...
        { 
            ...
            width: 140,
            editable: true,
            cellRenderer: params => params.value ? `👤 ${params.value}` : '<span class="text-gray-400 italic">Unassigned</span>'
        },
        {
            headerName: "Actions",
            width: 180,
    ...
    onCellValueChanged: (params) => {
        ...
        // Dispatch event to main.js for auto-saving inline edits
...
export function showLeadsView() {
    ...
    const waitForGrid = setInterval(() => {
        if (leadsGridApi) {
            ...
            console.log("[ui.js] Leads List Grid is ready. Attaching real-time listener.");
            const db = firebase.firestore();
            ...
            // 3. Real-time listener for the main collection
            unsubscribeLeadsListener = db.collection(LEADS_COLLECTION_PATH)
                .orderBy('enquiryDate', 'desc') // Changed from createdOn to enquiryDate for better relevance
                .onSnapshot(snapshot => {
...
const purchaseInvoicesGridOptions = {
    ...
    columnDefs: [
        ...
        { field: "supplierInvoiceNo", headerName: "Supplier Invoice #", width: 200 },
...
const catalogueItemsGridOptions = {
    ...
    columnDefs: [
        ...
        },
        {
            ...
            cellClass: 'flex items-center justify-center',
            cellRenderer: params => {
                const itemId = params.data.id || params.data.tempId;
                const deleteIcon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5"><path fill-rule="evenodd" d="M8.75 1A2.75 2.75 0 0 0 6 3.75v.443c-.795.077-1.58.22-2.365.468a.75.75 0 1 0 .23 1.482l.149-.022.841 10.518A2.75 2.75 0 0 0 7.596 19h4.807a2.75 2.75 0 0 0 2.742-2.53l.841-10.52.149.023a.75.75 0 0 0 .23-1.482A41.03 41.03 0 0 0 14 4.193V3.75A2.75 2.75 0 0 0 11.25 1h-2.5zM10 4c.84 0 1.673.025 2.5.075V3.75c0-.69-.56-1.25-1.25-1.25h-2.5c-.69 0-1.25.56-1.25 1.25v.325C8.327 4.025 9.16 4 10 4zM8.58 7.72a.75.75 0 0 0-1.5.06l.3 7.5a.75.75 0 1 0 1.5-.06l-.3-7.5zm4.34.06a.75.75 0 1 0-1.5-.06l-.3 7.5a.75.75 0 1 0 1.5.06l.3-7.5z" clip-rule="evenodd" /></svg>`;
...
const salesCartGridOptions = {
    ...
    columnDefs: [
        ...
        },
        {
            ...
            cellRenderer: params => {
                const removeIcon = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5"><path fill-rule="evenodd" d="M8.75 1A2.75 2.75 0 0 0 6 3.75v.443c-.795.077-1.58.22-2.365.468a.75.75 0 1 0 .23 1.482l.149-.022.841 10.518A2.75 2.75 0 0 0 7.596 19h4.807a2.75 2.75 0 0 0 2.742-2.53l.841-10.52.149.023a.75.75 0 0 0 .23-1.482A41.03 41.03 0 0 0 14 4.193V3.75A2.75 2.75 0 0 0 11.25 1h-2.5zM10 4c.84 0 1.673.025 2.5.075V3.75c0-.69-.56-1.25-1.25-1.25h-2.5c-.69 0-1.25.56-1.25 1.25v.325C8.327 4.025 9.16 4 10 4zM8.58 7.72a.75.75 0 0 0-1.5.06l.3 7.5a.75.75 0 1 0 1.5-.06l-.3-7.5zm4.34.06a.75.75 0 1 0-1.5-.06l-.3 7.5a.75.75 0 1 0 1.5.06l.3-7.5z" clip-rule="evenodd" /></svg>`;
                return `<button type="button" class="action-btn-icon action-btn-delete action-btn-remove-from-cart" data-id="${params.data.productId}" title="Remove Item">${removeIcon}</button>`;
            }
        }
    ],
    onCellValueChanged: () => {
    ...
    columnDefs: [
            ...
            suppressHeaderMenuButton: true },
        {
            ...
            cellRenderer: params => {
                ...
                return `<button class="action-btn-icon action-btn-add-to-cart" data-id="${productData.id}" data-product="${productDataString}" title="Add to Cart">${addIcon}</button>`;
            }
        }
    ],
    defaultColDef: {
...
async function loadAdminLandingDashboard(user, forceRefresh = false) {
    ...
    try {
        ...
        document.getElementById('sales-summary-total-expense').textContent = formatCurrency(sales.total.expenses);
        ...
        document.getElementById('sales-summary-consignment-expenses').textContent = formatCurrency(sales.consignment.consignmentExpenses);
...
function renderStockStatusTreemapold(stockData) {
    ...
    function drawTreemap(hoveredIndex = -1) {
        ...
        ctx.fillText('Stock Inventory Treemap', padding, 20);

        // Draw rectangles
        rectangles.forEach((item, idx) => {
    ...
    const handleMouseMove = function(e) {
        ...
        if (hoveredIndex !== canvasElement.treemapData.hoveredIndex) {
            canvasElement.treemapData.hoveredIndex = hoveredIndex;
...
function updateLeadProductGridByCatalogue(catalogueId) {
    ...
    // Filter master products by the selected catalogue ID
    const products = masterData.products
        ...
        .map(p => ({
            ...
            requestedQty: 0 // Default to zero for selection
        }));
...
function populateLeadDropdowns() {
    ...
    if (catSelect) {
        ...
        masterData.salesCatalogues.filter(c => c.isActive).forEach(cat => {
            catSelect.add(new Option(`${cat.catalogueName} (${cat.seasonName})`, cat.id));
        });
    }
...
export function closeLeadModal() {
    ...
    setTimeout(() => { 
        modal.style.display = 'none'; 
        // Clear grid data on close to ensure privacy/fresh start
        if (leadProductsGridApi) leadProductsGridApi.setGridOption('rowData', []);
    }, 300);
}
...
export async function processLeadToSaleConversion(leadData, selectedStore) {
    ...
    try {
        ...
        console.log("📦 [CONVERSION STEP 1] Package prepared:", conversionPackage);


        // 5. Switch to Sales View
        sessionStorage.setItem('pending_lead_conversion', JSON.stringify(conversionPackage));

        console.log("💾 [CONVERSION STEP 2] Saved to sessionStorage. Current value:", sessionStorage.getItem('pending_lead_conversion'));
        ...
        setTimeout(() => {
            console.log("⏱️ [CONVERSION STEP 3] Timeout finished. Calling autoFillSalesFormFromLead()...");
            autoFillSalesFormFromLead();
        }, 500);
...
function autoFillSalesFormFromLead() {
    console.log("🔍 [AUTOFILL STEP 4] Function triggered.");
    const rawData = sessionStorage.getItem('pending_lead_conversion');

    console.log("📄 [AUTOFILL STEP 5] Raw data found in storage:", rawData);


    if (!rawData) {
        console.warn("⚠️ [AUTOFILL] No data found in sessionStorage. Stopping.");
        return;
    }
    ...
    const waitForSalesElements = setInterval(() => {
        ...
        const emailInput = document.getElementById('sale-customer-email');
        ...
        console.log(`⏳ [AUTOFILL STEP 6] Checking elements: 
            ...
            - Catalogue Select: ${!!catSelect}
            - Grid API: ${!!cartGrid}`);

        // ✅ Check: Ensure dropdowns have been populated with options from masterData
        ...
        if (nameInput && isStorePopulated && isCatPopulated && cartGrid) {
            ...
            console.log("🚀 [AUTOFILL STEP 7] All elements and data ready. Injecting...");
            ...
            if (emailInput) emailInput.value = data.customerInfo.email || '';
            if (phoneInput) phoneInput.value = data.customerInfo.phone || '';
            
            // 3. Set Store and Trigger UI Logic (Address visibility)
            storeSelect.value = data.store;
            storeSelect.dispatchEvent(new Event('change')); 

            // 4. Set Address (if Tasty Treats)
            if (data.store === 'Tasty Treats') {
                const addressTextArea = document.getElementById('sale-customer-address');
                if (addressTextArea) {
                    addressTextArea.value = data.customerInfo.address || '';
                    // Ensure container is visible
                    document.getElementById('tasty-treats-address-container')?.classList.remove('hidden');
                }
            }

            // 5. Set Catalogue and Sale Type
            catSelect.value = data.catalogueId || '';
            
            if (typeSelect) {
                typeSelect.value = 'Revenue'; // Default to Revenue
                typeSelect.dispatchEvent(new Event('change')); // Trigger discount/sample logic
            }
...
const consignmentOrdersGridOptionsV2 = {
    ...
    columnDefs: [
        ...
        { field: "balanceDue", headerName: "Balance Due", valueFormatter: p => formatCurrency(p.value || 0) },
        {
            ...
            cellClass: 'flex items-center justify-center',
            cellRenderer: params => {
                // Define a clear, descriptive icon for opening/settling an order
                const settleIcon = `<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                    <path d="M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z" />
                    <path fill-rule="evenodd" d="M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" clip-rule="evenodd" />
                </svg>`;
                
                // Return a button with the icon, a tooltip, and the necessary data attributes
                return `<button class="text-blue-600 hover:text-blue-800 settle-consignment-btn-v2" data-id="${params.data.id}" title="Open / Settle Order">
                            ${settleIcon}
                        </button>`;
            }
            // --- END OF CHANGE ---
                    ...
                    if (isLocked) {
                        return `
                            <div class="flex items-center space-x-2">
                                ...
                                <span class="text-gray-500 font-medium">${params.value}</span>
                            </div>
                        `;
                    } else {
...
const consignmentPaymentHistoryGridOptionsV2 = {
    ...
    columnDefs: [
        ...
        { 
            ...
            width: 120, 
            cellRenderer: p => {
                const status = p.value || 'Verified';
                if (status === 'Voided') return `<span class="text-xs font-bold text-red-600 px-2 py-1 bg-red-100 rounded-full">VOIDED</span>`;
                if (status === 'Reversal') return `<span class="text-xs italic text-gray-500 px-2 py-1 bg-gray-100 rounded-full">REVERSAL</span>`;
                return `<span class="text-xs font-semibold text-green-600 px-2 py-1 bg-green-100 rounded-full">SUCCESS</span>`;
            }
        },

        // --- NEW: Actions Column (Void Button) ---
        {
            headerName: "Action",
            width: 80,
            cellClass: 'flex items-center justify-center',
            cellRenderer: params => {
                // Do not show the void button if the payment is already voided or is a reversal
                if (params.data.status === 'Voided' || params.data.status === 'Reversal') return '';

                const voidIcon = `<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd" />
                </svg>`;
                return `<button class="text-red-500 hover:text-red-700 void-payment-btn-v2" data-id="${params.data.id}" title="Void Payment">${voidIcon}</button>`;
            }
        }
    ],
