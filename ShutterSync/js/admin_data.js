...
export const AdminData = {
     ...
     */
    renderAdminDataUI: function(moduleContentElement) { // Changed parameter name for clarity
    ...
    saveAppSettings: async function() {
        if (!Auth.isLoggedIn() || !this.Utils.isAdmin()) {
            this.Utils.showMessage('You must be logged in as an Admin to save app settings.', 'error');
            return;
        }
        ...
        try {
            const appSettingsDocRef = doc(this.db, 'app_metadata', 'settings');
            await this.Utils.setDoc(appSettingsDocRef, settingsData, { merge: true }); // Use Utils.setDoc
            this.Utils.showMessage('Application settings saved successfully!', 'success');
        } catch (error) {
            this.Utils.handleError(error, "saving app settings");
        }
    },
