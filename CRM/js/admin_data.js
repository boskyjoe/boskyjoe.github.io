...
// Global data for Countries and States (fetched from Firestore)
...
// Helper function to get currency symbol by code (exported for other modules)
...
export async function initAdminDataModule(type) {
    ...
    if (!adminCountryMappingSection && type === 'country_mapping') {
        ...
        // Add event listeners for Country Mapping form
        if (document.getElementById('countryMappingForm')) {
            document.getElementById('countryMappingForm').addEventListener('submit', async (e) => {
                ...
                try {
                    ...
                    await fetchCountryData(); // Re-fetch data after successful admin update
    ...
    if (!currencyManagementSection && type === 'currency_management') {
        ...
        // Add event listeners for Currency Management form
        if (currencyForm) {
    ...
    // Ensure initial state and load data based on the type requested
